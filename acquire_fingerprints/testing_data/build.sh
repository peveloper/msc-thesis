...
get_adus() {
    ...
    if [ $1 == 0 ];
    then
        ../tools/adudump/adudump ../capture_log/$file -l 192.168.0.157/24 -q $adu_quiet_time > $adu_file
    fi
    
    declare -a local_ports=$(cut -d ' ' -f3 $adu_file | sort | uniq | cut -d '.' -f5)
    src_ips=$(cat ../capture_har/$file.har | cut -f1 | cut -d '/' -f3 | sort | uniq | xargs -l1 host | cut -d ' ' -f4)
...
#ls ../capture_log/ | parallel get_adus 0
#get_hars 
